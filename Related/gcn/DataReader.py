...
def create_node_feature(Edge):
    ...
    for i in range(len(Edge)):
        ...
        G_katz = nx.algorithms.centrality.katz_centrality_numpy(
            G, alpha=0.1, beta=1.0, normalized=True, weight=None
        )
        G_closeness = nx.algorithms.centrality.closeness_centrality(
            G, u=None, distance=None, wf_improved=False
        )
        G_betweeness = nx.algorithms.centrality.betweenness_centrality(
            G, k=None, normalized=True, weight=None, endpoints=False, seed=None
        )
        G_page_rank = nx.pagerank(
            G, alpha=0.85, max_iter=100, tol=1e-08, nstart=None
        )
